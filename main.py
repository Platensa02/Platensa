...
async def handler(message: Message):
    ...
    if state == "admin" and message.text.isdigit():
        ...
        temp_data[user_id] = {"target": int(message.text)}
        await message.answer("Nechta dona?")
        return

    # ADMIN AMOUNT
    if state == "admin" and user_id in temp_data and message.text.isdigit():

        temp_data[user_id]["amount"] = int(message.text)

        await message.answer(
            "Tasdiqlaysizmi?",
            reply_markup=confirm_keyboard(
                "add",
                temp_data[user_id]["target"],
                temp_data[user_id]["amount"]
            )
        )
        return
